...
│
├── artifacts/                         ← Precomputed outputs. Mixed: some committed, some gitignored (too large).
│   ├── embeddings.npy                 ← All candidate embeddings. (n_candidates × 768, fp16). Gitignored.
│   ├── candidate_ids.npy              ← Aligned array of CAND_XXXXXXX IDs. Gitignored.
│   ├── jd_vector.npy                  ← JD embedding vector (1 × 768, fp16). Committed.
│   ├── ltr_model.txt                  ← Trained LightGBM LambdaMART model. Committed.
│   ├── ce_scores.json                 ← {candidate_id: raw_logit} cross-encoder scores. Committed.
│   └── citations.json                 ← Reasoning citation map, written by rank.py at run time. Committed.
│
├── src/
...
│   ├── fast_filter.py                 ← Stage 2 cascade gates (cheap rule-based filtering before LTR scoring). Used by rank.py.
│   ├── ownership_classifier.py        ← Regex-based "did they build it or just touch it" intent scorer. Used by feature_builder.py.
│   ├── expand_query.py                ← Persona-based JD query expansion via Claude. Used by precompute.py (optional, offline).
│   ├── counterfactual.py              ← Track 2 / explainability. Per-feature rank-impact audit. Not imported by rank.py.
│   ├── hiring_recommendation.py       ← Track 2 / explainability. Tiers a candidate into a recruiter-facing recommendation. Not imported by rank.py.
│   └── role_analyzer.py               ← Track 2 / explainability. Converts role_model.yaml into a recruiter-readable summary. Not imported by rank.py.
│
├── scripts/
│   ├── precompute_cross_encoder.py    ← OFFLINE, GPU-only (Colab). Generates artifacts/ce_scores.json.
│   └── generate_rich_reasoning.py     ← Track 2 / explainability. Generates extended reasoning via Gemini for the dashboard.
│
├── eval/
│   ├── golden_set.csv                 ← Hand-labeled candidates (157 rows). Ground truth.
...
All files in this directory are generated by `precompute.py`, `train_ltr.py`, `scripts/precompute_cross_encoder.py`, or `rank.py` itself. Not all of them are gitignored:

- **Gitignored** (too large): `embeddings.npy`, `candidate_ids.npy`, `sample_embeddings.npy`, `sample_candidate_ids.npy` — regenerate with `precompute.py`.
- **Committed** (small enough to track, needed for a clean checkout to run `rank.py` against the sample artifacts without a precompute pass): `ltr_model.txt`, `jd_vector.npy`, `ce_scores.json`, `citations.json`.

The `README.md` must document how to regenerate the gitignored ones.
...
The ground truth for local evaluation. 157 candidate profiles hand-labeled against the JD. This is the only way to measure ranking quality without the hidden ground truth.
